...
const PriceComparer = ({ ItemToSearch }) => {
  ...
  const fetchBudget = async () => {
    try {
      const response = await Axios.get('http://localhost:3000/users/budget', {
  ...
  const handleSubtractFromBudget = async () => {
    try {
      ...
      setBudget(updateResponse.data.updatedBudget);
    } catch (error) {
      console.error('Error subtracting from budget:', error);
    }
  };
  ...
  const generatePDF = () => {
    ...
    selectedItems.forEach((item, index) => {
      ...
      pdf.text(
        `Promo Price: $${item.price[0].promo !== 0 ? item.price[0].promo : 'N/A'}`,
        20,
        yPosition + 30
      );
    });
    ...
    <div style={{ backgroundColor: 'tan' }}>
      ...
      <div>
        ...
        <div className="search-section">
          ...
          <div>
            ...
            {items.map((item) => (
              ...
              > <h2> <a  href={`https://www.kroger.com/search?query=${item.productId}&searchType=default_search`}  target="_blank" rel="noopener noreferrer">
                    {item.brand || 'Kroger'}
                  </a>
                </h2>
        ...
        <div className="selected-section">
          ...
          <button type="button" onClick={handleSubtractFromBudget}>Subtract from Budget</button>
